...
My design choices for the counter clock forces my minimum and maximum frequency calculations to be the following:
    ...
    Calculation for minimum frequency supported:
        This is calculated by first considering the the counter's input clock frequency (10 KHz) and bit width of the counter (16 bits as discussed in the manual on page 887). By assuming that the counter will reach 65536, we can find the value for when the MCU will output its lowest freqeuncy given the configurations:

![Minimum Freqeuncy Supported](images/min_freq.jpg)

    Calculation for maximum freqeuncy supported:
        This is calculated very similarly to the minimum frequency calculation. However, not the amount of times the counter has clicked will no longer be the counter's maximum value but rather its minimum value - 1:

![Maximum Frequency Supported](images/max_freq.jpg)

Now we will cover how I implemented a delay function for a given duration. It is a similar process to how I implemented the PWM mode for timer 16. This time around I used timer 15, and put the timer in its upcounting mode to strictly increment. Most of the same registers used for implementing the PWM signal were used for the delay function with an exception for a handful that were used for comparison registers. I created initialization and update fucntions for the delay where the former would only ever update those registers once while the update function would consistently refresh the values in severl registers. The major difference between the PWM and delay functions was that there was a while loop with the delay update function checking for when a flag would be set when the duration limit had been satisfied. At that point, the while loop would end and the flag would be reset for the next delay then the next line would run. 

The PSC calculation for the delay register is the same as the one in the PWM register to avoid becoming an asynchronous design too much. 
...
My design choices for the counter clock forces my minimum and maximum duration calculations to be the following:

    Calculation for minimum duration supported:
        This is the reciprocol of the input counter clock, 10 KHz.
...
This is a link to my {{< video https://youtu.be/yFY1lExnWCY width="250" height="175" >}} that plays Fur Elise. 

This is a link to my Happy Birthday song {{< video https://youtu.be/Cy9B9i0Aetk width="250" height="175" >}}. 

These are calculations that show that the individual pitches are accurate to within 1%. The measured values seen in the images are captured from an oscilloscope.
